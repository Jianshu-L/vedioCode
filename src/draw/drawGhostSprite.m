...
global gameWindow ghosts;
global DIR_UP DIR_RIGHT DIR_DOWN DIR_LEFT scale;
window = gameWindow;
x = ghosts(i).pixel.x;
y = ghosts(i).pixel.y;
frame = mod(floor(ghosts(i).frames/8),2);
dirEnum = ghosts(i).dirEnum;
scared = ghosts(i).scared;
flash = isEnergizerFlash;
eyes_only = ghosts(i).mode == GHOST_GOING_HOME || ghosts(i).mode == GHOST_ENTERING_HOME;
